...
router.post('/', authMiddleware, requireRole('student'), async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO applications (position_id, student_id, cover_letter)
       VALUES ($1, $2, $3)
       RETURNING *`,
      [positionId, student.id, coverLetter ?? null]
    );
...
router.get('/mine', authMiddleware, requireRole('student'), async (req: Request, res: Response) => {
  const studentResult = await pool.query('SELECT id FROM student_profiles WHERE user_id = $1', [req.userId]);
  const student = studentResult.rows[0];
  if (!student) {
    return res.json([]);
  ...
  const result = await pool.query(
    `SELECT a.*, p.title as position_title, pp.lab_name
     FROM applications a
...
router.get('/position/:id', authMiddleware, requireRole('pi'), async (req: Request, res: Response) => {
  ...
  const result = await pool.query(
    `SELECT a.*, sp.major, sp.gpa, sp.skills, sp.bio, sp.resume_url, sp.year_level,
...
router.patch('/:id/status', authMiddleware, requireRole('pi'), async (req: Request, res: Response) => {
  const { id } = req.params;
  const { status } = req.body;
  if (!status || !['pending', 'reviewed', 'accepted', 'rejected', 'withdrawn'].includes(status)) {
    return res.status(400).json({ error: 'Valid status required: pending, reviewed, accepted, rejected, withdrawn' });
  }
  const piResult = await pool.query('SELECT id FROM pi_profiles WHERE user_id = $1', [req.userId]);
  const pi = piResult.rows[0];
  if (!pi) {
