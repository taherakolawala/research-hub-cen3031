...
router.post('/', authMiddleware, requireRole('student'), asyncHandler(async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO applications (position_id, student_id, personal_statement)
       VALUES ($1, $2, $3)
       RETURNING *`, [positionId, student.id, statement]);
...
router.get('/mine', authMiddleware, requireRole('student'), asyncHandler(async (req, res) => {
    const studentResult = await pool.query('SELECT id FROM student_profiles WHERE user_id = $1', [req.userId]);
    const student = studentResult.rows[0];
    if (!student) {
        return res.json([]);
    }
    const result = await pool.query(`SELECT a.*, rp.title as position_title, pp.lab_name
     FROM applications a
...
router.get('/position/:id', authMiddleware, requireRole('pi'), asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(`SELECT a.*, sp.major, sp.gpa, sp.skills, sp.bio, sp.resume_url, sp.academic_level,
...
router.patch('/:id/status', authMiddleware, requireRole('pi'), asyncHandler(async (req, res) => {
    const { id } = req.params;
