...
router.post('/', authMiddleware, requireRole('pi'), async (req, res) => {
    ...
    const result = await pool.query(`INSERT INTO positions (pi_id, title, description, required_skills, min_gpa, is_funded, deadline)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     RETURNING *`, [pi.id, title, description ?? null, requiredSkills ?? [], minGpa ?? null, isFunded ?? false, deadline ?? null]);
...
router.get('/mine', authMiddleware, requireRole('pi'), async (req, res) => {
    const piResult = await pool.query('SELECT id FROM pi_profiles WHERE user_id = $1', [req.userId]);
    const pi = piResult.rows[0];
    if (!pi) {
        return res.json([]);
...
router.get('/:id', async (req, res) => {
    const { id } = req.params;
    const result = await pool.query(`SELECT p.*, pp.department, pp.lab_name, pp.research_area, pp.lab_website
     FROM positions p
     JOIN pi_profiles pp ON pp.id = p.pi_id
     WHERE p.id = $1`, [id]);
    const row = result.rows[0];
...
router.put('/:id', authMiddleware, requireRole('pi'), async (req, res) => {
    const { id } = req.params;
    ...
    const result = await pool.query(`UPDATE positions SET
     ...
     RETURNING *`, [title ?? null, description ?? null, requiredSkills ?? null, minGpa ?? null, isFunded ?? null, isOpen ?? null, deadline ?? null, id, pi.id]);
    const row = result.rows[0];
    if (!row) {
...
router.delete('/:id', authMiddleware, requireRole('pi'), async (req, res) => {
    const { id } = req.params;
    const piResult = await pool.query('SELECT id FROM pi_profiles WHERE user_id = $1', [req.userId]);
    const pi = piResult.rows[0];
    if (!pi) {
    ...
    const result = await pool.query(`UPDATE positions SET is_open = false WHERE id = $1 AND pi_id = $2 RETURNING id`, [id, pi.id]);
